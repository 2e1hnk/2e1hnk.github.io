...
distBetweenGridSquares = function(grid1, grid2) {
  gridSquareToLatLon(gridSquare1, grid1);
  gridSquareToLatLon(gridSquare2, grid2);
  var R = 6371; // Radius of the earth in km
  var dLat = deg2rad(grid2.lat-grid1.lat);  // deg2rad below
  var dLon = deg2rad(grid2.lon-grid1.lon);
  var a =
    Math.sin(dLat/2) * Math.sin(dLat/2) +
    Math.cos(deg2rad(grid1.lat)) * Math.cos(deg2rad(grid2.lat)) *
...
distBetweenLocations = function(lat1,lon1,lat2,lon2) {
  var R = 6371; // Radius of the earth in km
